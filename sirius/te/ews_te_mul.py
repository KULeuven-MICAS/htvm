...
def intrin_ews(ro,co,data_type):
    ...
    def intrin_func(ins, outs):
        ...
        #
        # Both input_buffers and output_buffer are of type tvm.tir.Buffer
        A, B = ins
        C = outs[0]
        # We build an expression that reads from the input buffers.
        A_read_expr = A.vload([0, 0])
        B_read_expr = B.vload([0, 0])
