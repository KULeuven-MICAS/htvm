...
def schedule_conv2d(outs):
    ...
    def _callback(op):
        ...
        if "conv2d_nhwc" in op.tag:
            ...
            data_orig = s[op].op.input_tensors[0].op.input_tensors[0]
            # Get padded tensor and tensorize padding
            s.stages[1].tensorize(op.input_tensors[0].op.axis[0], intrin_pad_NHWC(data_orig.shape, kernel_orig.shape))
            # Now also tensorize the conv2d operation without the padding
            s.stages[3].tensorize(op.axis[1], intrin_conv2d_hwlib_NHWC(data_orig.shape, kernel_orig.shape))
